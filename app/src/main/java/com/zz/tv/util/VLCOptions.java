...
import com.zz.tv.VLCApplication;
...
public class VLCOptions {
    ...
    public static ArrayList<String> getLibOptions() {
        ...
        options.add("--no-rtsp-tcp");
        options.add("--rtsp-frame-buffer-size=50");
        return options;
    }
...
//        }
        if (hardwareAcceleration == HW_ACCELERATION_DISABLED) {
            ...
            Log.d(TAG, "setMediaOptions " + "HW_ACCELERATION_DISABLED");
        } else if (hardwareAcceleration == HW_ACCELERATION_FULL || hardwareAcceleration == HW_ACCELERATION_DECODING) {
            media.setHWDecoderEnabled(true, true);
            Log.d(TAG, "setMediaOptions " + "HW_ACCELERATION_FULL");
            if (hardwareAcceleration == HW_ACCELERATION_DECODING) {
                Log.d(TAG, "setMediaOptions " + "HW_ACCELERATION_DECODING");
                media.addOption(":no-mediacodec-dr");
                media.addOption(":no-omxil-dr");
            }
        }  else {/* else automatic: use default options */

        }
        if (noVideo) {
            media.addOption(":no-video");
            Log.d(TAG, "setMediaOptions " + "no-video");
        }

        if (paused) {
            media.addOption(":start-paused");
            Log.d(TAG, "setMediaOptions " + "start-paused");
        }
    }
